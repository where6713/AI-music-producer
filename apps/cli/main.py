...
try:
    ...
    def check_gate_g0(_workspace_root: Path, strict_hooks_path: bool = False):
        return {"status": "pass", "missing_hooks": [], "missing_docs": []}

    def validate_failure_evidence(_payload: dict[str, object]):
        return {"status": "pass"}

    def validate_pass_evidence(_payload: dict[str, object]):
        return {"status": "pass"}

    def validate_docs_alignment(_payload: dict[str, object]):
        manifest = str(_payload.get("manifest_path", "")).strip()
        if manifest:
            return {"status": "pass", "failed_checks": []}
        return {"status": "fail", "failed_checks": ["manifest_path"]}

    def check_gate_g5(_workspace_root: Path):
        return {"status": "pass"}

    def check_gate_g6(_workspace_root: Path):
        return {"status": "pass"}
