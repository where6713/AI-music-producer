...
if [ -n "$root_files" ]; then
  root_whitelist='^(AI-music-producer PRD_v1\.1\.md|one law\.md|开发清单\.md|目录框架规范\.md|README\.md|LICENSE|CHANGELOG\.md|\.gitignore)$'
  disallowed_root="$(printf '%s\n' "$root_files" | grep -Ev "$root_whitelist" || true)"
