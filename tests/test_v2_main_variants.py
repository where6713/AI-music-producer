...
def test_produce_fails_quality_floor_and_skips_lyrics_write(tmp_path, monkeypatch) -> None:
    ...
    observed = {"write_called": 0}

    def _fake_write_outputs(*_args, **_kwargs):
        observed["write_called"] += 1
    ...
    assert calls["n"] == 3
    assert observed["write_called"] == 1


def test_quality_floor_still_fails_loud_when_structure_empty(tmp_path, monkeypatch) -> None:
    from src import main as main_mod

    payload = _payload()
    payload.lyrics_by_section = []
    payload.variants[0].lyrics_by_section = []
    payload.variants[1].lyrics_by_section = []
    payload.variants[2].lyrics_by_section = []
...
def test_quality_regression_rollback_when_failed_rules_increase() -> None:
    from src import main as main_mod

    before = {"failed_rules": ["R01"], "is_dead": False}
    after = {"failed_rules": ["R01", "R02"], "is_dead": False}
    assert main_mod._is_quality_regression(before, after) is True


def test_quality_regression_no_rollback_when_not_worse() -> None:
    from src import main as main_mod

    before = {"failed_rules": ["R01", "R02"], "is_dead": False}
    after = {"failed_rules": ["R01"], "is_dead": False}
    assert main_mod._is_quality_regression(before, after) is False
